...
<div class="min-h-screen bg-gray-100 pt-20">
    ...
    <div class="max-w-4xl mx-auto p-8">
        <div class="mb-6">
            <h1 class="text-2xl font-bold text-gray-800 mb-2">แก้ไขข้อมูลประกาศ</h1>
            <p class="text-gray-600">กรอกข้อมูลประกาศที่ต้องการแก้ไข</p>
        </div>

        <!-- Edit Announcement -->
        <section>
            <form action="{{ route('announcement.update', ['activity' => $activity]) }}" method="POST" class="bg-white rounded-xl shadow-sm border border-gray-200 p-6" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-2 gap-6 mb-5">
                    <div class="space-y-2">
                        ...
                        <input
                            ...
                            value="{{ old('join_start_datetime', $activity->join_start_datetime ? date('Y-m-d\\TH:i', strtotime($activity->join_start_datetime)) : '') }}"
                    ...
                    <div class="space-y-2">
                        ...
                        <input
                            ...
                            value="{{ old('join_end_datetime', $activity->join_end_datetime ? date('Y-m-d\\TH:i', strtotime($activity->join_end_datetime)) : '') }}"
                        ...
                        <select id="condition" name="condition" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500">
                            ...
                            @foreach ($conditions as $condition)
                                <option value="{{ $condition }}"
                                    {{ old('condition', $activity->condition) == $condition ? 'selected' : '' }}>
                                    {{ $condition }}
                                </option>
                            @endforeach
                        </select>
    ...
    document.getElementById('addActivityTypeBtn').addEventListener('click', function () {
        ...
        if (newValue) {
            ...
            if (!exists) {
                ...
                alert('This type already exists in the dropdown.');
            }
            newTypeInput.value = '';
        } else {
    ...
    document.getElementById('addConditionBtn').addEventListener('click', function () {
        ...
        if (newValue) {
            ...
            if (!exists) {
                ...
                alert('This condition already exists in the dropdown.');
            }
